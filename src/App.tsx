...
// --- Helper chips ---
const Chip: React.FC<{ children: React.ReactNode; tone?: "neutral" | "success" | "warning" | "danger" }>
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[var(--text-mini)] font-medium ${tones[tone]}`}>{children}</span>
  );
};

// --- KPI Card ---
const KpiCard: React.FC<{ title: string; value: string; helper?: string; icon?: React.ReactNode }>
  ...
  <div className="group relative rounded-2xl bg-white shadow-sm p-5 border border-black/5 hover:shadow-md hover:-translate-y-[1px] transition-[transform,box-shadow] duration-300">
    <div className="flex items-start justify-between">
      <div>
        ...
        <h3 className="mt-2 text-3xl font-semibold" style={{ color: "var(--primary-color)" }}>{value}</h3>
      </div>
...
// --- Table Row ---
const TaskRow: React.FC<{ idx: number; title: string; owner: string; status: "En curso" | "Completada" | "Bloqueada"; due: string }>
  = ({ idx, title, owner, status, due }) => {
  const tone = status === "Completada" ? "success" : status === "Bloqueada" ? "danger" : "warning";
  const Icon = status === "Completada" ? CheckCircle2 : status === "Bloqueada" ? AlertTriangle : Clock;
  return (
    <tr className="border-b last:border-b-0">
      ...
      <td className="py-3 px-3"><Chip tone={tone as any}><Icon size={14} /> {status}</Chip></td>
      <td className="py-3 px-3 text-right"><span className="text-sm text-black/70">{due}</span></td>
    </tr>
  );
};

// --- Activity Item ---
const ActivityItem: React.FC<{ title: string; time: string; icon: React.ReactNode; hint?: string }>
  = ({ title, time, icon, hint }) => (
  <div className="flex gap-3 items-start">
    <div className="mt-0.5 shrink-0 rounded-xl p-2 bg-white border border-black/5 shadow-sm">{icon}</div>
    <div className="min-w-0">
      <p className="text-sm" style={{ color: "var(--primary-color)" }}>{title}</p>
...
// --- DATA (mock) ---
const TASKS = [
  { title: "Actualizar política de vacaciones 2026", owner: "RR.HH.", status: "En curso" as const,    due: "15 Nov 2025" },
  { title: "Cierre de sprint #12 – Intranet Cintax", owner: "TI", status: "Completada" as const,        due: "08 Nov 2025" },
  { title: "Revisión de contratos de proveedores", owner: "Legal", status: "Bloqueada" as const,       due: "20 Nov 2025" },
  { title: "Capacitación de inducción (cohorte 11)", owner: "Personas", status: "En curso" as const,   due: "22 Nov 2025" },
];

const ANNOUNCEMENTS = [
  { title: "Feriado institucional 02 Dic", copy: "El 02 de diciembre habrá cierre parcial. Revise turnos.", cta: "Ver calendario" },
